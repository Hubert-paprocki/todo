...
const Task: React.FC<TaskProps> = ({
  ...
  const getTimeLeft = (): string | null => {
    ...
    if (taskDeadlineDate.toDateString() === currentDate.toDateString()) {
      return `${context.selectedLanguage.task.taskDateToday}`;
    }
    ...
    if (daysLeft < 0) {
      ...
      return `${daysLeft} ${context.selectedLanguage.task.taskDateUpcoming} (${deadlineString})`;
    }
  };
    ...
    <li className={classes}>
      {isUpdating ? (
        <>
          ...
          <div className=" md:row-span-3 md:col-span-1 text-center md:pr-4 justify-self-center flex">
          ...
          <div className="flex-1 text-gray-400 col-start-2 row-start-2 md:row-start-1  md:col-start-3 md:row-span-3 flex items-center justify-evenly ">
            <p className="text-stone-500 dark:text-gray-400">
              {context.selectedLanguage.task.taskSetNewDate}
            </p>
            <div className="">
        ...
        <>
          ...
          <p className="flex-1 text-stone-600 dark:text-gray-400 col-start-2 row-start-2 md:row-start-1 justify-self-center md:col-start-3 md:row-span-3 flex items-center px-2">
            {!isTaskOverdue()
              ? getTimeLeft()
              : `${
                  context.selectedLanguage.task.taskDateMissed
                } ${getTimeLeft()}`}
          </p>
